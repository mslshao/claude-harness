...
- Hardcoded external reference to the wrong env → an endpoint/DNS URL, API gateway ID, account ID, or secret ARN hardcoded in a non-prod env that actually targets prod (or a shared account the per-env account cannot reach cross-account). Smell: the same value (a `/prod`-staged URL, a prod domain, a shared-account ARN) appears in cd/beta and prod alike. Resolve from infra: grep for per-env existence (`infra/{service}/{env}`) and compare account IDs across the env terragrunt files. Consequence: non-prod tokens 401 against prod, or (worse) non-prod compute reads the prod corpus. Sweep EVERY sibling field of the same kind (`*_dns_name`, `*_secret_arn`) before concluding, not just the one flagged. When infra alone cannot confirm which account/env an opaque endpoint (e.g. an `execute-api` id) targets, emit the finding as needs-confirmation and note the Datadog `aws_account`/`env` span check for the orchestrator to run (this agent has no Datadog access)
...
- Shared-module EXPRESSION change under review (for-expression, conditional, jsonencode transform) → do NOT sign off from reasoning about runtime semantics; render it. HCL type-unifies BOTH branches of a conditional even when a branch is never taken, so `cond ? v : flatten([...jsondecode...])` fails plan with "Inconsistent conditional result types" while every runtime-discrimination argument holds (`bd recall gotcha:hcl-conditional-type-unification`; PR #11390: two devops review passes reasoned this correct, a rendered plan disproved it in minutes). Rendering needs NO AWS access and is NOT blocked by the codespace terragrunt restrictions: copy the module to scratch (drop provider-heavy files like monitors.tf), instantiate it with the real call-site value shapes, stub provider (fake keys + skip_credentials_validation + skip_requesting_account_id + skip_metadata_api_check + TF_PLUGIN_CACHE_DIR=~/.terraform.d/plugin-cache), then `terraform plan -out` + `terraform show -json` and read the rendered attribute back. When byte-identity is the claim, render old-vs-new per shape and diff the strings.
