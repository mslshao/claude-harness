...
You flag but don't deep-dive (one line + routing note, then move on):
- Missing type annotations in test code → flag for style attention
- Security-relevant test gaps (auth bypass, PII leakage) → flag for security review
- Test infrastructure problems (fixture failures, moto config) → flag for build/deploy attention
...
The fix is to remove the mock and test through the front door: call the real code, then verify the outcome by querying the (moto-faked) service directly. For HTTP calls, explicitly use the `responses` fixture (or `@responses.activate`) to register expected endpoints. For async HTTP via `aiohttp`, use `aioresponses`.
...
Verification: grep the test file for `model_dump_json`, `json.dumps`, or `mode='json'`. If absent and the source module publishes events (EventBridge, SQS, SNS) or returns API responses, flag. Cross-ref: `code-reviewer`'s Pydantic enum serialization pattern.
...
↗️ Flag for security review: test_auth.py tests token parsing but not expiration/revocation
```
