...
Phase 4) is what the user reads.
...
Surface every override explicitly in the Phase 4 rationale.
...
Fold the skeptic output into the Phase 4 presentation under the
`### Skeptic Lens` section. See SKILL.md Phase 4 template for section
ordering. If the skeptic returns `🔻 No concerns from this lens.`,
still include the section with that exact line so the user can see
the pass ran.
...
the skeptic pass on whatever exists (top-1 or top-2). The pass still
runs; the prompt's "three candidate approaches" wording becomes "two
candidate approaches" or "the single candidate".

### Failure handling

If the skeptic dispatch fails (agent missing, transient error), note
the failure in the Phase 4 output as a one-line "Skeptic Lens
unavailable: <reason>" and proceed. Do not block ideation on advisory
tooling, but do not silently drop the pass either; the user must know
the adversarial check did not run.

The skeptic findings (or the unavailable-reason) appear in the Phase 4
presentation under `### Skeptic Lens`. A missing skeptic pass is
itself a signal the human reading the output should weigh: a winner
that was not adversarially checked carries lower confidence, and the
iterate-or-proceed call should account for that.
